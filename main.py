...
def connect():
    ...
    try:
        ...
        data, addr = data_gen.sock.recvfrom(
            1500
...
with dpg.file_dialog(
    modal=True,
    show=False,
    callback=open_values,
    id="open_values_file_picker",
    width=500,
    height=400,
...
with dpg.window(label="Graph", autosize=True, pos=(125, 175)):
    dpg.add_slider_int(
        label="Gearbox", min_value=2, max_value=10, source="gearbox", width=62
    )
...
with dpg.window(
    ...
    dpg.add_button(
        label="Close",
        width=75,
        callback=lambda: dpg.configure_item("error_window", show=False),
    )
...
with dpg.window(
    ...
    dpg.add_button(
        label="Close",
        width=75,
        callback=lambda: dpg.configure_item("status_window", show=False),
    )

with dpg.viewport_menu_bar():
    with dpg.menu(label="Logs"):
        dpg.add_menu_item(
            label="Open", callback=lambda: dpg.show_item("open_values_file_picker")
        )
...
with dpg.window(
    ...
    dpg.add_button(
        label="Close",
        width=75,
        callback=lambda: dpg.configure_item("save_window", show=False),
    )
...
with dpg.theme() as global_theme:
    with dpg.theme_component(dpg.mvAll):
        ...
        dpg.add_theme_style(
            dpg.mvStyleVar_GrabRounding, 5, category=dpg.mvThemeCat_Core
        )
